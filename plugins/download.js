...
cmd({
    ...
    async (conn, mek, m, { from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
        try {
            ...
            reply(`*♕︎ 𝙲𝚈𝙱𝙴𝚁 𝙿𝙷𝙾𝙴𝙽𝙸𝚇 𝘎𝘋𝘙𝘐𝘝𝘌 𝘋𝘖𝘞𝘕𝘓𝘖𝘈𝘋𝘌𝘙* \n\n*📃 File name:*  ${res.fileName}
*💈 File Size:* ${res.fileSize}
*🕹️ File type:* ${res.mimetype}

*•ᴄʏʙᴇʀ-ᴘʜᴏᴇɴɪx ᴡʜᴀᴛsᴀᴘᴘ ʙᴏᴛ* `)
            conn.sendMessage(from, { document: { url: res.downloadUrl }, fileName: res.fileName, mimetype: res.mimetype, caption: res.fileName.replace('[Cinesubz.co]', '[phonix-MOVIES.]') + '\n\n> *•ᴄʏʙᴇʀ-ᴘʜᴏᴇɴɪx ᴡʜᴀᴛsᴀᴘᴘ ʙᴏᴛ•*' }, { quoted: mek })
        } catch (e) {
            reply('*Error !!*')
            l(e)
        }
...
cmd({
    ...
    async (conn, mek, m, {
        ...
        try {
            ...
            let caption = `🎶 *PHONIX MD SONG DOWNLOADER* 🎶
...
cmd({
    ...
    async (conn, mek, m, { from, q, reply }) => {
        if (!q) return await reply('🎵 *Please provide a song name! (e.g., .lyrics Sandawathiye)*');

        try {
            // Initial fetching message
            let loadingMsg = await reply('🔍 *Fetching lyrics...* ⏳\n[░░░░░░░░░░] 0%');
            ...
            const res = await fetchJson(`https://apis.prexzyvilla.site/search/lyrics?title=${encodeURIComponent(q)}`);
            ...
            if (res && res.status && res.data) {
                ...
                let lyricsMessage = `🎵 *${data.title || 'N/A'}*\n\n`;
                lyricsMessage += `👤 *Artist:* ${data.artist || 'N/A'}\n`;
                lyricsMessage += `💿 *Album:* ${data.album || 'N/A'}\n`;
                lyricsMessage += `⏱️ *Duration:* ${formattedDuration}\n\n`;
                lyricsMessage += `📝 *Lyrics:*\n${data.lyrics || 'No lyrics found!'}\n\n`;
                lyricsMessage += `⚡ *Powered by PHOENIX MD*`;
            ...
            console.log(e);
            await reply('❌ *Error occurred while fetching lyrics.*');
        }
    })
...
cmd({
    ...
    async (conn, mek, m, { from, q, reply }) => {
        if (!q) return await reply('🖼️ *Please provide a search query! (e.g., .wallpaper nature)*');

        try {
            // Initial fetching message
            let loadingMsg = await reply('🔍 *Fetching wallpapers...* ⏳\n[░░░░░░░░░░] 0%');
            ...
            const res = await fetchJson(`https://apis.prexzyvilla.site/search/wallpaper?query=${encodeURIComponent(q)}`);

            if (res && res.status && res.data && res.data.results && res.data.results.length > 0) {
                const query = res.data.query || q;
                const count = res.data.count || res.data.results.length;
                const results = res.data.results;

                // Determine how many images to send (max 4)
                const imagesToSend = Math.min(results.length, 4);

                await conn.sendMessage(from, {
                    text: `✅ *Wallpapers Found!*\n\n🔎 *Query:* ${query}\n📊 *Total Results:* ${count}\n📤 *Sending first ${imagesToSend} wallpaper(s)...*`,
                    edit: loadingMsg.key
                });

                // Send up to 4 wallpapers
                for (let i = 0; i < imagesToSend; i++) {
                    await conn.sendMessage(from, {
                        image: { url: results[i] },
                        caption: `🖼️ *Wallpaper ${i + 1}/${imagesToSend}*\n🔎 *Query:* ${query}\n\n⚡ *Powered by PHOENIX MD*`
                    });
                ...
                // If there are more results, inform user
                if (count > 4) {
                    await conn.sendMessage(from, {
                        text: `📌 *${count - 4} more wallpapers available!*\n💡 *Tip: Try a more specific query for better results.*`
                    });
                }

            } else {
                await conn.sendMessage(from, {
                    text: '❌ *No wallpapers found for this query!*',
                    edit: loadingMsg.key
                });
            }
        } catch (e) {
            console.log(e);
            await reply('❌ *Error occurred while fetching wallpapers.*');
        }
    })

// Separate handler for message replies
conn.ev.on('messages.upsert', async (mek) => {
    const m = mek.messages[0];
    if (!m.message) return;

    const msg = m.message.extendedTextMessage || m.message.conversation;
    if (!msg) return;

    // Check if it's a reply to wallpaper prompt
    ...
    if (!global.wallpaperCache || !global.wallpaperCache[sender]) return;

    const cache = global.wallpaperCache[sender];

    // Check if replying to the correct message
    if (cache.msgId !== quotedId) return;

    // Check if cache is expired (5 minutes)
    if (Date.now() - cache.timestamp > 300000) {
        delete global.wallpaperCache[sender];
        await conn.sendMessage(sender, { text: '❌ *Session expired! Please search again.*' });
        return;
    }

    const userReply = msg.text || m.message.conversation || '';
    const requestedCount = parseInt(userReply.trim());

    // Validate input
    if (isNaN(requestedCount) || requestedCount < 1 || requestedCount > cache.results.length) {
        await conn.sendMessage(sender, { text: `❌ *Please reply with a valid number between 1 and ${cache.results.length}!*` });
        return;
    }

    try {
        await conn.sendMessage(sender, { text: `⏳ *Please wait... Sending ${requestedCount} wallpaper(s)...*` });

        // Send requested number of wallpapers
        for (let i = 0; i < requestedCount; i++) {
            await conn.sendMessage(sender, {
                image: { url: cache.results[i] },
                caption: `🖼️ *Wallpaper ${i + 1}/${requestedCount}*\n🔎 *Query:* ${cache.query}\n\n⚡ *Powered by PHOENIX MD*`
            });
            // Small delay between images to avoid rate limiting
        ...
        console.log(e);
        await conn.sendMessage(sender, { text: '❌ *Error occurred while sending wallpapers.*' });
        delete global.wallpaperCache[sender];
    }
});
...
cmd({
    ...
    async (conn, mek, m, { from, l, prefix, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
        try {
            ...
            let caption = `*\`🪺 PHONIX MD TIK TOK DOWNLODER 🪺\`*
...
cmd({
    ...
    async (conn, mek, m, { from, q, reply }) => {
        if (!q) return await reply('🤖 *Please provide a prompt! (e.g., .ai hello)*');

        try {
            const res = await fetchJson(`https://apis.prexzyvilla.site/ai/ai4chat?prompt=${encodeURIComponent(q)}`);

            if (res && res.status && res.data && res.data.response) {
                await reply(res.data.response);
            } else {
                await reply('❌ *No response from AI!*');
            }
        } catch (e) {
            console.log(e);
            await reply('❌ *Error occurred while fetching AI response.*');
            console.log(e)
        }
    })
...
cmd({
    ...
    try {
        ...
        const caption = `\`🏮 PHONIX MD FB DOWNLOADER 🏮\`\n\n` +
            `*┌──────────────────*\n` +
            `*├ \`🐼 Title:\`* ${title}\n` +
            `*├ \`⏱️ Duration:\`* ${duration}\n` +
            `*├ \`🔗 Url:\`* ${q}\n` +
...
cmd({
    ...
    async (conn, mek, m, { from, q, reply }) => {
        try {
            ...
            let listdata = `*\`🗃️ PHONIX MD APK DOWNLOADER 🗃️\`*
...
cmd({
    ...
    async (conn, mek, m, { from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
        try {
            ...
            let listdata = `*\`🔥 𝙲𝚈𝙱𝙴𝚁 𝙿𝙷𝙾𝙴𝙽𝙸𝚇 𝙈𝙀𝘿𝙄𝘼𝙁𝙄𝙍𝙀 𝘿𝙊𝙒𝙉𝙇𝙊𝘿𝙀𝙍 🔥\`*
...
cmd({
    ...
    async (conn, m, mek, { from, q, isMe, prefix, reply }) => {
        try {
            ...
            let msg = `*\`🔞 𝙲𝚈𝙱𝙴𝚁 𝙿𝙷𝙾𝙴𝙽𝙸𝚇 𝙋𝙊𝙍𝙉𝙃𝙐𝘽 𝘿𝙊𝙒𝙉𝙇𝙊𝘼𝘿𝙀𝙍 🔞\`*
...
cmd({
    ...
    async (conn, mek, m, { from, prefix, q, reply }) => {
        try {
            ...
            const caption = `*\`🎼 PHONIX MD SOUNDCLOUD DOWNLOADER 🎼\`*\n\n*🎶 Title:* ${data.title}\n*🔗 URL:* ${q}`;
            ...
            const buttonMessage = {
                ...
                caption: caption,
                footer: config.FOOTER || 'PHONIX MD BOT',
                buttons: buttons,
                headerType: 4
            };
